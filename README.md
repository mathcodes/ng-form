# Angular Forms and Things

### In-Depth explorations into the `ResultsComponent`
...
interface User {
  team: string; // this is a TypeScript interface that is used to define the structure of an object
  id: number;
  name: string;
  email: string;
  city: string;
...
export class ResultsComponent implements OnInit {
  teamAUsers: User[] = []; // Define teamAUsers property with type User[]
  teamBUsers: User[] = []; // Define teamBUsers property with type User[]
  ...
  loadUsers(): void {
    ...
    this.http.get<User[]>(endpoint)
      .subscribe(
        (response) => {
          this.teamAUsers = response.filter(user => user.team === 'Team A');
          this.teamBUsers = response.filter(user => user.team === 'Team B');
        },
        (error) => {
